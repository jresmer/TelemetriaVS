...
void write_data (float** x, float* y, float* w, int size, int d, int dataset_size) {
    ...
    FILE* file;
    file = fopen("labeled_data.txt", "ab");
    if (file == NULL) {
        fprintf(stderr, "\nError opening file\n");
        exit(1);
    }
    ...
    for (int i = 0; i < size; i++) {
        flag = fwrite(&w[i], sizeof(float), 1, file);
        if (!flag) {
...
void generate_quadratic_data () {
    float w[2] = {rand() % 1000 / rand() % 1000, rand() % 1000 / rand() % 1000};
    float x[1000][1];
    float y[1000];
...
void generate_exponential_data () {
    float w[3] = {rand() % 1000 / rand() % 1000, rand() % 1000 / rand() % 1000, rand() % 1000 / rand() % 1000};
    float x[1000][1];
    float y[1000];
